...
<section class="container mx-auto px-5 py-12">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @forelse ($category->topics as $topic)
            <div class="group bg-white rounded-2xl border border-gray-150 p-6 shadow-sm hover:shadow-xl hover:border-blue-500 transition-all duration-300 flex flex-col justify-between min-h-[180px]">
                <div>
                ...
                <div class="mt-6 pt-4 border-t border-gray-100 flex items-center justify-between">
                    <div class="flex items-center gap-1.5 text-sm text-gray-500 font-medium">
                        <span>📝</span>
                        <span>{{ $topic->words_count ?? '15' }} từ vựng</span>
                    </div>

                    <a href="{{ route('categories.show', $category->slug) }}/{{ $topic->slug ?? $topic->id }}" 
